...
module.exports = (bot) => {
  // APPROVE payment for $199 plan
  bot.action(/approve199_(\d+)/, async (ctx) => {
    ...
    try {
      ...
      await ctx.telegram.sendMessage(
        ...
        "✅ Your payment has been successfully approved!\n\nWelcome to the VIP group.",
        {
          reply_markup: {
            inline_keyboard: [
              [{ text: "Join VIP Group", url: config.VIP_GROUP_LINK_199 }]
            ]
          }
        }
      );
  ...
  // REJECT payment for $199 plan
  bot.action(/reject199_(\d+)/, async (ctx) => {
    const userId = ctx.match[1];
